...
(function(){
  ...
  UnobtrusiveFlash.showFlashMessage = function(message, options) {
    ...
    var $flashContainer = window.UnobtrusiveFlash.containerHtml;
    if ($flashContainer.closest(document.documentElement).length==0) { // Not yet in DOM
      $flashContainer.prependTo('body');
    }

    var $flash = window.UnobtrusiveFlash.flashHtml.clone().addClass('unobtrusive-flash-' + options.type).html(message).appendTo($flashContainer);
